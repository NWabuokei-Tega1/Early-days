...
print(d)
#tuple function to arrange words from shortest to longest. words of same length are printed randomly- lines 6 & 9
import random
def sort_words(words):
	y = []
	for i in words:
		y.append((len(i), random.random(), i ))
	y.sort(reverse = False)	
	x = []
	for length,_, i in y:
		x.append(i)
	return(x)
words = ["pineapple", "periwinkles", "sugar", "acorn", "apple"]
print(sort_words(words))
#inbuilt functions like sorted and reversed, help to sort tuples in ascending or descending order
t4 = ('a', 'x','h')
print(sorted(t4))
